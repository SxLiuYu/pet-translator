...
async def upload_audio(file: UploadFile = File(...)):
    ...
    try:
        ...
        result = BehaviorResult(**{**classification, **analysis})
...
async def get_events(limit: int = 50):
    """获取最近的行为事件"""
    events = behavior_engine.daily_events[-limit:]
    return {"events": [e.to_dict() for e in events], "total": len(behavior_engine.daily_events)}
...
async def register_camera(req: CameraRegisterRequest):
    ...
    try:
        ...
        cam.start()
        return {"status": "started", "name": req.name, "type": req.source_type}
    except Exception as e:
...
'''
Path('server/app.py').write_text(text, encoding='utf-8')
print('wrote', len(text.splitlines()), 'lines')
